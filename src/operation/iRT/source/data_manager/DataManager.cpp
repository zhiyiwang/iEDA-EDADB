...
void DataManager::outputJson()
{
  ...
  json_path_map["input_snapshot"] = outputInputSnapshotJson();
...
std::string DataManager::outputInputSnapshotJson()
{
  auto planar_rect_json = [](PlanarRect& rect) {
    return nlohmann::json::array({rect.get_ll_x(), rect.get_ll_y(), rect.get_ur_x(), rect.get_ur_y()});
  };
  auto ext_rect_json = [&planar_rect_json](EXTPlanarRect& rect) {
    return nlohmann::json{{"real", planar_rect_json(rect.get_real_rect())}, {"grid", planar_rect_json(rect.get_grid_rect())}};
  };
  auto layer_rect_json = [&planar_rect_json](LayerRect& rect) {
    return nlohmann::json{{"rect", planar_rect_json(rect)}, {"layer_idx", rect.get_layer_idx()}};
  };
  auto ext_layer_rect_json = [&ext_rect_json](EXTLayerRect& rect) {
    nlohmann::json rect_json = ext_rect_json(rect);
    rect_json["layer_idx"] = rect.get_layer_idx();
    return rect_json;
  };
  auto scale_grid_json = [](ScaleGrid& grid) {
    return nlohmann::json::array({grid.get_start_line(), grid.get_step_length(), grid.get_step_num(), grid.get_end_line()});
  };
  auto scale_axis_json = [&scale_grid_json](ScaleAxis& axis) {
    nlohmann::json axis_json = {{"x", nlohmann::json::array()}, {"y", nlohmann::json::array()}};
    for (ScaleGrid& grid : axis.get_x_grid_list()) {
      axis_json["x"].push_back(scale_grid_json(grid));
    }
    for (ScaleGrid& grid : axis.get_y_grid_list()) {
      axis_json["y"].push_back(scale_grid_json(grid));
    }
    return axis_json;
  };

  nlohmann::json database_json;
  database_json["design_name"] = _database.get_design_name();
  database_json["micron_dbu"] = _database.get_micron_dbu();
  database_json["manufacture_grid"] = _database.get_manufacture_grid();
  database_json["detection_distance"] = _database.get_detection_distance();
  database_json["gcell_axis"] = scale_axis_json(_database.get_gcell_axis());
  database_json["die"] = ext_rect_json(_database.get_die());
  database_json["row"] = {_database.get_row().get_start_x(), _database.get_row().get_start_y(), _database.get_row().get_height()};

  database_json["routing_layers"] = nlohmann::json::array();
  for (RoutingLayer& layer : _database.get_routing_layer_list()) {
    SpacingTable& spacing_table = layer.get_prl_spacing_table();
    GridMap<int32_t>& spacing_map = spacing_table.get_width_parallel_length_map();
    nlohmann::json spacing_values = nlohmann::json::array();
    for (int32_t x = 0; x < spacing_map.get_x_size(); x++) {
      nlohmann::json row_json = nlohmann::json::array();
      for (int32_t y = 0; y < spacing_map.get_y_size(); y++) {
        row_json.push_back(spacing_map[x][y]);
      }
      spacing_values.push_back(row_json);
    }
    database_json["routing_layers"].push_back({
        {"idx", layer.get_layer_idx()},
        {"order", layer.get_layer_order()},
        {"name", layer.get_layer_name()},
        {"prefer_direction", static_cast<int32_t>(layer.get_prefer_direction())},
        {"track_axis", scale_axis_json(layer.get_track_axis())},
        {"min_width", layer.get_min_width()},
        {"min_area", layer.get_min_area()},
        {"notch_spacing", layer.get_notch_spacing()},
        {"spacing_widths", spacing_table.get_width_list()},
        {"spacing_parallel_lengths", spacing_table.get_parallel_length_list()},
        {"spacing_values", spacing_values},
        {"eol_spacing", layer.get_eol_spacing()},
        {"eol_ete", layer.get_eol_ete()},
        {"eol_within", layer.get_eol_within()},
    });
  }

  database_json["cut_layers"] = nlohmann::json::array();
  for (CutLayer& layer : _database.get_cut_layer_list()) {
    database_json["cut_layers"].push_back({
        {"idx", layer.get_layer_idx()},
        {"order", layer.get_layer_order()},
        {"name", layer.get_layer_name()},
        {"curr_spacing", layer.get_curr_spacing()},
        {"curr_prl", layer.get_curr_prl()},
        {"curr_prl_spacing", layer.get_curr_prl_spacing()},
        {"curr_eol_spacing", layer.get_curr_eol_spacing()},
        {"curr_eol_prl", layer.get_curr_eol_prl()},
        {"curr_eol_prl_spacing", layer.get_curr_eol_prl_spacing()},
        {"above_spacing", layer.get_above_spacing()},
        {"above_prl", layer.get_above_prl()},
        {"above_prl_spacing", layer.get_above_prl_spacing()},
        {"below_spacing", layer.get_below_spacing()},
        {"below_prl", layer.get_below_prl()},
        {"below_prl_spacing", layer.get_below_prl_spacing()},
    });
  }

  auto int_map_json = [](std::map<int32_t, int32_t>& value_map) {
    nlohmann::json map_json = nlohmann::json::array();
    for (auto& [key, value] : value_map) {
      map_json.push_back({key, value});
    }
    return map_json;
  };
  auto int_vector_map_json = [](std::map<int32_t, std::vector<int32_t>>& value_map) {
    nlohmann::json map_json = nlohmann::json::array();
    for (auto& [key, value] : value_map) {
      map_json.push_back({key, value});
    }
  ...
  database_json["routing_idb_layer_id_to_idx"] = int_map_json(_database.get_routing_idb_layer_id_to_idx_map());
  database_json["cut_idb_layer_id_to_idx"] = int_map_json(_database.get_cut_idb_layer_id_to_idx_map());
  database_json["routing_to_adjacent_cut"] = int_vector_map_json(_database.get_routing_to_adjacent_cut_map());
  database_json["cut_to_adjacent_routing"] = int_vector_map_json(_database.get_cut_to_adjacent_routing_map());
  database_json["routing_layer_name_to_idx"] = nlohmann::json::array();
  for (auto& [name, idx] : _database.get_routing_layer_name_to_idx_map()) {
    database_json["routing_layer_name_to_idx"].push_back({name, idx});
  }
  database_json["cut_layer_name_to_idx"] = nlohmann::json::array();
  for (auto& [name, idx] : _database.get_cut_layer_name_to_idx_map()) {
    database_json["cut_layer_name_to_idx"].push_back({name, idx});
  }

  database_json["via_masters"] = nlohmann::json::array();
  for (std::vector<ViaMaster>& via_master_list : _database.get_layer_via_master_list()) {
    nlohmann::json via_master_list_json = nlohmann::json::array();
    for (ViaMaster& via_master : via_master_list) {
      nlohmann::json cut_shapes_json = nlohmann::json::array();
      for (PlanarRect& cut_shape : via_master.get_cut_shape_list()) {
        cut_shapes_json.push_back(planar_rect_json(cut_shape));
      }
      ViaMasterIdx& via_master_idx = via_master.get_via_master_idx();
      via_master_list_json.push_back({
          {"below_layer_idx", via_master_idx.get_below_layer_idx()},
          {"via_idx", via_master_idx.get_via_idx()},
          {"name", via_master.get_via_name()},
          {"above_enclosure", layer_rect_json(via_master.get_above_enclosure())},
          {"above_direction", static_cast<int32_t>(via_master.get_above_direction())},
          {"below_enclosure", layer_rect_json(via_master.get_below_enclosure())},
          {"below_direction", static_cast<int32_t>(via_master.get_below_direction())},
          {"cut_shapes", cut_shapes_json},
          {"cut_layer_idx", via_master.get_cut_layer_idx()},
      });
    }
    database_json["via_masters"].push_back(via_master_list_json);
  }

  database_json["layer_enclosures"] = nlohmann::json::array();
  for (auto& [layer_idx, enclosure] : _database.get_layer_enclosure_map()) {
    database_json["layer_enclosures"].push_back({layer_idx, planar_rect_json(enclosure)});
  }
  database_json["layer_cut_shapes"] = nlohmann::json::array();
  for (auto& [layer_idx, cut_shape] : _database.get_layer_cut_shape_map()) {
    database_json["layer_cut_shapes"].push_back({layer_idx, planar_rect_json(cut_shape)});
  }

  database_json["routing_obstacles"] = nlohmann::json::array();
  for (Obstacle& obstacle : _database.get_routing_obstacle_list()) {
    database_json["routing_obstacles"].push_back(ext_layer_rect_json(obstacle));
  }
  database_json["cut_obstacles"] = nlohmann::json::array();
  for (Obstacle& obstacle : _database.get_cut_obstacle_list()) {
    database_json["cut_obstacles"].push_back(ext_layer_rect_json(obstacle));
  }

  database_json["nets"] = nlohmann::json::array();
  for (Net& net : _database.get_net_list()) {
    nlohmann::json net_json = {
        {"idx", net.get_net_idx()},
        {"name", net.get_net_name()},
        {"connect_type", static_cast<int32_t>(net.get_connect_type())},
        {"bounding_box", ext_rect_json(net.get_bounding_box())},
        {"pins", nlohmann::json::array()},
    };
    for (Pin& pin : net.get_pin_list()) {
      nlohmann::json routing_shapes_json = nlohmann::json::array();
      for (EXTLayerRect& shape : pin.get_routing_shape_list()) {
        routing_shapes_json.push_back(ext_layer_rect_json(shape));
      }
      nlohmann::json cut_shapes_json = nlohmann::json::array();
      for (EXTLayerRect& shape : pin.get_cut_shape_list()) {
        cut_shapes_json.push_back(ext_layer_rect_json(shape));
      }
      AccessPoint& access_point = pin.get_access_point();
      net_json["pins"].push_back({
          {"idx", pin.get_pin_idx()},
          {"name", pin.get_pin_name()},
          {"is_driven", pin.get_is_driven()},
          {"routing_shapes", routing_shapes_json},
          {"cut_shapes", cut_shapes_json},
          {"access_point",
           {
               {"pin_idx", access_point.get_pin_idx()},
               {"real", {access_point.get_real_x(), access_point.get_real_y()}},
               {"grid", {access_point.get_grid_x(), access_point.get_grid_y()}},
               {"layer_idx", access_point.get_layer_idx()},
           }},
      });
    }
    database_json["nets"].push_back(net_json);
  }

  GridMap<GCell>& gcell_map = _database.get_gcell_map();
  database_json["gcell_map_size"] = {gcell_map.get_x_size(), gcell_map.get_y_size()};

  nlohmann::json fixed_rect_order_json = nlohmann::json::array();
  for (auto& [is_routing, layer_net_fixed_rect_map] : getTypeLayerNetFixedRectMap(_database.get_die())) {
    for (auto& [layer_idx, net_fixed_rect_map] : layer_net_fixed_rect_map) {
      for (auto& [net_idx, fixed_rect_set] : net_fixed_rect_map) {
        nlohmann::json rect_list_json = nlohmann::json::array();
        for (EXTLayerRect* fixed_rect : fixed_rect_set) {
          rect_list_json.push_back(ext_layer_rect_json(*fixed_rect));
        }
        fixed_rect_order_json.push_back({
            {"is_routing", is_routing},
            {"layer_idx", layer_idx},
            {"net_idx", net_idx},
            {"rects", rect_list_json},
        });
      }
    }
  }
  ...
  nlohmann::json snapshot_json = {
      {"schema_version", 1},
      {"semantic_database", database_json},
      {"pointer_order_views", {{"fixed_rects", fixed_rect_order_json}}},
  };
  std::string snapshot_file_path = RTUTIL.getString(_config.dm_temp_directory_path, "input_snapshot.json");
  std::ofstream* snapshot_file = RTUTIL.getOutputFileStream(snapshot_file_path);
  (*snapshot_file) << snapshot_json;
  RTUTIL.closeFileStream(snapshot_file);
