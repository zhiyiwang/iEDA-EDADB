...
bool DefReadEdadb::createDbByDef(const char* path) {
    ...
    // Demo branch: these object families are not restored from EDADB yet, so
    // keep their original DEF parser callbacks enabled.
...
bool DefReadEdadb::createDbByEdadb(const char* edadb_path) {
    EDADB_IDB_DEBUG_STREAM << "[EDADB-IDB] createDbByEdadb demo Design/Die/Row/TrackGrid/GCell/Region/Slot enabled path="
              << edadb_path << std::endl;
