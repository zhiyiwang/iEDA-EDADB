# EDADB iDB Adapter Development Rules

本文是 iEDA DEF read/write 迁移到 EDADB read/write 的唯一实现、review 与文档规则来源。测试规则和结果只维护在 `adapter-testing.md`；逐类结论位于 `idb-adapter/`；README 只提供索引。
...
测试模型、fixture 规则、并发策略、命令、断言和结果只维护在 `adapter-testing.md`。本文件只要求实现变更同步更新相应测试，不复制测试正文。
...
14. 按 `adapter-testing.md` 为受影响分支补齐合法 fixture 和 acceptance gates。
15. 重新核对文档行号、Markdown table 列数和 `git diff --check`。

## Order / Index Authority

Root order levels, point-tool evidence, implementation status and planned order-stress experiments are maintained only in `def-ieda-mapping-and-order.md`.

## Progress Authority

The enabled demo scope is maintained in `README.md`; the per-class sequence is maintained in `idb-adapter/README.md`. Do not duplicate class status tables here.
...
## Class Index Authority

Read `idb-adapter/README.md` for the ordered 01-14 class index. Each class document owns only its class-specific schema, field mapping, reconstruction and coverage evidence.
