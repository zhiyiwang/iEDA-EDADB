...
**状态：已冻结。** 代码、实验定义及本轮结论未经用户明确同意不再修改；规则见[AGENTS.md](AGENTS.md)。后续实验独立进行，不覆盖本批结果。
...
- 后续主键/索引研究见独立的[sqlite-index](../sqlite-index/readme.md)：当前为单表实验，不包含FK；状态与结果在其目录维护，不并入本实验。
