...
参数仅在[sqlite-reference/config.md](../sqlite-reference/config.md)维护：
...
python3 scripts/edadb/performance/storage/baseline/run.py \
...
python3 scripts/edadb/performance/storage/baseline/audit.py /tmp/iedadb_baseline_full
```
...
- NULL／参数清理对照已完成，见[SQLite与EDADB对照结果](../sqlite-vs-edadb/results.md)；不把其他分支优化结果混入本基线。
