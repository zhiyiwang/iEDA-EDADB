...
- B-batch：文件DB、本批实际DELETE/FULL、批量事务；OS-warm新连接读取。[配置说明](../sqlite-reference/config.md)
...
cmake -S scripts/edadb/performance/storage/sqlite-vs-edadb \
...
# 小规模检查，输出目录须不存在
python3 scripts/edadb/performance/storage/sqlite-vs-edadb/run_alignment.py \
...
python3 scripts/edadb/performance/storage/sqlite-vs-edadb/audit_alignment.py \
