...
上述读写对照已完成，共16组独立正确性检查、80条正式计时；此次整理重新审计原始结果，没有重复执行相同实验。已验证的两项是读端NULL检查、写端参数清理；尚未解决的是其余框架成本的独立量化，不能为了凑齐总时间而推算各函数耗时。

- 默认配置的选择：disk+journal+synchronous 配置
- pk fk 多表的性能差异：复合pk主键，复合index，无index
- adapter性能拆分：shadow转换，1:N 嵌套的代价，多prepare执行？
- N+1的修改：batch？BFS？
